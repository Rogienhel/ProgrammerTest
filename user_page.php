                    ...
                    if($result){
                      while($row = mysqli_fetch_assoc($result)){
                        ...
                        echo '<tr>
                                    ...
                                    <td>'.$position.'</td>
                                    <td>';
                                    if($_GET['userID'] == $id && $user_type == "user")
                                      {
                                        if($status == "active"){echo '<a class = "status" id = "active" href = "include/logout.php?userID='.$id.'&status=deactive">Active</a></td>';}
                                        else{echo '<a class = "status" id = "deactivate" href = "include/status.php?userID='.$id.'&status=active">Activate</a></td>';}
                                        echo
                                      '<td>
                                        <a href = "edit_page.php?editID='.$id.'" id = "btn" class = "fas fa-pencil-alt btn-edit"></a>
                                        <a href = "view_page.php?viewID='.$id.'" id = "btn" class = "fas fa-id-card btn-view"></a>
                              ...
                              ';}
                                      elseif ($user_type == "admin") {
                                        echo ''.$status.'</td><td>Not Applicable!!!</td></tr>';
                                      }
                                      else
                                      {
                                        echo ''.$status.'</td><td>
                      ...
                      if($resultCheck < 0){echo '<tr><td colspan = "10">No results found!</td></tr>';}
                        ...
                        while($row = mysqli_fetch_assoc($result)){
                          ...
                          echo '<tr>
                                      ...
                                      <td>'.$position.'</td>
                                      <td>';
                                      if($_GET['userID'] != $id && $user_type != "admin")
                                      {
                                        if($status == "active"){echo '<a class = "status" id = "active" href = "include/logout.php?userID='.$id.'&status=deactive">Active</a></td>';}
                                        else{echo '<a class = "status" id = "deactivate" href = "include/status.php?userID='.$id.'&status=active">Activate</a></td>';}
                                    echo
